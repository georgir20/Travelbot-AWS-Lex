...
def main_calc_destination_station_state(destination_station_state):
# use city from user input and query table in athena database
	client = boto3.client('athena')

	query_string= "SELECT DISTINCT current_station_name FROM trains_in_india WHERE current_station_state ='" + destination_station_state +"'"
...
# open new csv query result and read into lex bot
	...
	if not rows :
		...
		return 'The train stations that we found for ' + destination_station_state + ' are: {}.'.format(rows) + 'Do you want to set one of those stations as your destination station? '
...
def return_StationName(intent_request):
	...
	return close(
		...
		'Fulfilled',
		{
			...
			'content': main_calc_destination_station_state(destination_station_state) 
...
def dispatch(intent_request):
    ...
    intent_name='GetDestinationStationState'
    
    # Dispatch to your bot's intent handlers
    if intent_name == 'GetDestinationStationState':
